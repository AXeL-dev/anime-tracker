...
export class HTMLParserService {
  ...
  parse(
    ...
    const scopeSelector = isArray(scope)
      ? (scope as string[]).join(', ')
      : (scope as string);
    dom.querySelectorAll(scopeSelector).forEach((item) => {
      const data = {};
      Object.keys(selector).forEach((key: string) => {
        // String
        if (isString(selector[key])) {
          data[key] = this.find(item, selector[key] as string, filters);
        }
        // Array
        else if (isArray(selector[key])) {
          data[key] = [];
          (selector[key] as []).forEach((element: any) => {
            if (isString(element)) {
  ...
  find(element: any, selector: string, filters?: FilterList) {
